...
class funcoes():
    ...
    def cadastrar_cliente (self):
        ...
        if (self.nome == "" and self.email =="" and self.telefone=="" and self.cidade=="" and self.idmt5=="" and  self.vencimento =="" and self.corretora ==""):
            messagebox.showinfo(title="CADASTRO ERROR", message="Preencha todos os campos")
            ...
            self.conn.commit()
            messagebox.showinfo(title="CONCLUIDO!", message=f"CLIENTE {self.nome} CADASTRADO COM SUCESSO!")
    ...
    def alterar_cliente(self):
        ...
        if  self.nome != "" and self.email !=""  and self.telefone !="" and self.cidade !="" and self.idmt5 !="" and self.senhamt5 !="" and self.vencimento !="" and self.corretora !="":
            self.cursor.execute(""" UPDATE clientes SET nome = ?, email = ?, telefone = ?, cidade = ?, idmt5 = ?, senhamt5 = ?, vencimento = ?, corretora = ?
            WHERE cod = ?""",( self.nome, self.email, self.telefone, self.cidade, self.idmt5, self.senhamt5, self.vencimento, self.corretora, self.cod))
            self.conn.commit()
            messagebox.showinfo(title="CONCLUIDO!", message=f"Cadastro de {self.nome} atualizado com sucesso!")
            ...
            messagebox.showinfo(title="ALTERAR ERROR", message="Dê um duplo click encima do cliente para selecionar\n, atualizar ou apagar!")
        
    #Fazer backup das tabelas e cadastros
    def Backup_clientes(self, file_name='backup/clientes_bkp.sql'):
        self.conectar_db()
        with io.open(file_name, 'w') as f:
            for linha in self.conn.iterdump():
                f.write('%s\n' % linha)

        messagebox.showinfo(title='Backup Concluido', message='Backup realizado com sucesso.\nSalvo como %s' % file_name)
...
class apaplication(funcoes, relatorios):
    ...
    #Tela principal
    def tela(self):
        self.root.title('PAGINA DE VENDAS')
    ...
    def Menus (self):
        ...
        filemenu.add_command(label="Fazer Backup", command= self.Backup_clientes)
